...
import * as duelsRepo from "./duels.repository";
import * as questionsRepo from "../questions/questions.repository";
import {
    toActiveDuelDto,
    toDuelDetailDto,
    toDuelAnswerDetailDto,
    type ActiveDuelDto,
    type DuelDetailDto,
    type DuelAnswerDetailDto,
} from "./duels.mapper";
...
export async function getActiveDuel(params: {
    userId: string;
}): Promise<ServiceResult<{ duel: ActiveDuelDto } | null>> {
    const { userId } = params;
    ...
    return ok({
        duel: toActiveDuelDto(duel, !!alreadySubmitted),
    });
}
...
export async function joinAndStartDuel(params: {
    ...
    logger.info({ duelId, count: questionIds.length, theme }, "Duel: locking questions");
...
export async function getDuel(params: {
    userId: string;
    duelId: string;
}): Promise<ServiceResult<{ duel: DuelDetailDto }>> {
    const duel = await duelsRepo.findByIdForUser(params.duelId, params.userId);

    if (!duel) return err(404, "Duel not found");

    return ok({ duel: toDuelDetailDto(duel) });
}
...
export async function submitDuelAnswers(params: {
    ...
    const details = answers.map((a) => {
        ...
        const detail = toDuelAnswerDetailDto(question, a);
